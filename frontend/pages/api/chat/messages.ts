    ...
    if (req.method === "GET") {
      ...
      if (!conversationKey) {
        return res.status(400).json({
          error: "Conversation Key is required for fetching messages",
        });
      }
      try {
        const messages = await (Messages as any)
          .find({
            conversationKey,
          })
          .exec();
      ...
      let { conversationKey } = req.query;
      if (!conversationKey) {
        return res
          .status(400)
          .json({ error: "Conversation Key is required to delete messages" });
      }
      if (Array.isArray(conversationKey)) {
        conversationKey = conversationKey[0];
      }
      try {
        await (Messages as any).deleteMany({
          conversationKey: String(conversationKey),
        });
        return res
          .status(200)
          .json({ message: "Messages have been successfully deleted" });
      } catch (error) {
